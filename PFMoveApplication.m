...
void PFMoveToApplicationsFolderIfNecessary()
{
	...
	NSAlert *alert = [[[NSAlert alloc] init] autorelease];
	{
		...
		informativeText = NSLocalizedStringFromTable(@"Scrawl can be moved to the Applications folder automatically if you'd like.", @"MoveApplication", nil);
	...
	if ([alert runModal] == NSAlertFirstButtonReturn) {
		...
		NSLog(@"Moving Scrawl to the Applications folder");
		...
		if (needAuthorization) {
			...
			if (!AuthorizedInstall(bundlePath, destinationPath, &authorizationCanceled)) {
				if (authorizationCanceled) {
					...
					NSLog(@"ERROR -- Could not copy Scrawl to /Applications with authorization");
					goto fail;
				}
			}
		}
			...
			if ([fm fileExistsAtPath:destinationPath]) {
 			...
 			if (!CopyBundle(bundlePath, destinationPath)) {
				NSLog(@"ERROR -- Could not copy Scrawl to /Applications");
				goto fail;
			}
		}
